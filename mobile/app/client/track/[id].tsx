...
export default function TrackScreen() {
  ...
  const [riderName, setRiderName] = useState("Courier");
    ...
    if (data.rider_id) {
      ...
      if (rider?.user_id) {
        const { data: user } = await supabase
          ...
          .eq("id", rider.user_id)
          .maybeSingle();
        setRiderName(user?.full_name?.split(" ")[0] ?? "Courier");
  ...
  const submitRating = async (rating: number) => {
    ...
    try {
      const { data: rider } = await supabase
        ...
        .maybeSingle();
      if (!rider?.user_id) throw new Error("Courier unavailable for rating");
      ...
      top={
        <View style={styles.topRow}>
          ...
          <Pressable
            ...
            onPress={() =>
              Share.share({
                message: `Track my Gratitude Ride package ${delivery.tracking_id}`,
              })
            }
          >
        ...
        >
          ...
          {finding ? (
            ...
            <ContactBar
              name={riderName}
              phone={riderPhone}
              subtitle={`Courier · ${formatCurrency(delivery.estimated_fee)}`}
            />
          )}
